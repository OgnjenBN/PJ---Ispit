...
@router.get("/{device_type_id}", response_model=schemas.MyDevice)
def get_read_device(device_type_id: int, db: Session = Depends(get_db)):
    read_device = db.query(models.Device).filter(models.Device.id == device_type_id).first()
    if not read_device:
        raise HTTPException(status_code=404, detail="Tip uređaja nije pronađen")
    return read_device


@router.post("/", response_model=schemas.MyDevice)
def create_device_type(device_type: schemas.MyDeviceCreate, db: Session = Depends(get_db)):
